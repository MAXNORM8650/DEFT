...
from diffusers import AutoencoderKL, DDPMScheduler, DPMSolverMultistepScheduler, StableDiffusionXLPipeline, UNet2DConditionModel
...
from modules.main import KnowledgeInjectionLayer, InjectionMethod
...
# Call cleanup() at the end of the script or when done with distributed tasks.
...
DEFAULT_PARAMS = dict(
    ...
    dreambooth_deft_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_deftQR_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_deft_GFR8_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_deft_GFR4_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_para_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_deftp_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
    dreambooth_deftpo_sdxl=dict(guidance_scale=7.5, num_inference_steps=100, negative_prompt=None),
...
def dreambench_plus(
    ...
    if method == "blip_diffusion":
        ...
        #     # create a default AttnProcessor for every attention block
        #     proc_dict = unet.build_attn_processor_dict(AttnProcessor)
        #     unet.set_attn_processor(proc_dict)                # now the attr exists
        ...
        #    # Set the `lora_layer` attribute of the attention-related matrices.
            attn_module.to_q.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_q, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_k.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_k, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_v.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_v, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_out[0].set_knowledge_injection_layer(
                KnowledgeInjectionLayer(
                    base_layer = attn_module.to_out[0],
                    injection_method = InjectionMethod.RESIDUAL_PROJECTION,
        ...
        for attn_processor_name, attn_processor in unet.attn_processors.items():
            ...
            for n in attn_processor_name.split(".")[:-1]:
                attn_module = getattr(attn_module, n)
        #    # Set the `lora_layer` attribute of the attention-related matrices.
            attn_module.to_q.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_q, r=4, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_k.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_k, r=4, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_v.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_v, r=4, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=False))
            attn_module.to_out[0].set_knowledge_injection_layer(
                KnowledgeInjectionLayer(
                    base_layer = attn_module.to_out[0],
                    r=4,
        ...
        for attn_processor_name, attn_processor in unet.attn_processors.items():
            ...
            for n in attn_processor_name.split(".")[:-1]:
                attn_module = getattr(attn_module, n)
        #    # Set the `lora_layer` attribute of the attention-related matrices.
            attn_module.to_q.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_q, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=True,  use_gating="0", r = 4))
            attn_module.to_k.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_k, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=True,  use_gating="0", r = 4))
            attn_module.to_v.set_knowledge_injection_layer(KnowledgeInjectionLayer(base_layer = attn_module.to_v, injection_method = InjectionMethod.RESIDUAL_PROJECTION, orthonormalize=True,  use_gating="0", r = 4))
            attn_module.to_out[0].set_knowledge_injection_layer(
                KnowledgeInjectionLayer(
                    base_layer = attn_module.to_out[0],
                    injection_method = InjectionMethod.RESIDUAL_PROJECTION,
    ...
    elif method == "dreambooth_deftpo_sdxl":
        assert db_or_ti_output_dir is not None, "`db_or_ti_output_dir` must be provided"
        pipeline = StableDiffusionXLPipeline.from_pretrained(
            MODEL_ZOOS["stabilityai/stable-diffusion-xl-base-1.0"],
            torch_dtype=torch_dtype,
    ...
    # logger.info("Pipeline initialization complete.")  # after pipeline setup

    # Add this line before the main image generation loop
    logger.info(f"Processing {len(dreambench_plus)} samples.")
    params = []
    # breakpoint()
    for sample in dreambench_plus:
        if (
            ...
            or method == "dreambooth_deft_sdxl"
            or method == "dreambooth_deftQR_sdxl"
            or method == "dreambooth_deft_GFR8_sdxl"
            or method == "dreambooth_deft_GFR4_sdxl"
            or method == "dreambooth_para_sdxl"
            or method == "dreambooth_deftp_sdxl"
            or method == "dreambooth_deftpo_sdxl"
            or method == "emu2"
        ):
            for i, (text_prompt_input, prompt_gt) in enumerate(zip(sample.captions, sample.captions)):
                params.append((sample.collection_id, sample.subject, sample.image, i, text_prompt_input, prompt_gt))
                logger.info(f"Generating image {i+1}/{len(sample.captions)} for collection {sample.collection_id}")
        elif method == "emu2":
            pass
    ...
    with distributed_state.split_between_processes(params) as sub_params:
        for _param in sub_params:
            ...
            if method == "blip_diffusion":
                ...
                prompt = text_prompt_input
                pipeline.load_lora_weights(os.path.join(db_or_ti_output_dir, id, f"checkpoint-{steps}/"))
                pipe_kwargs = dict(height=1024, width=1024)
            elif method == "dreambooth_deft_sdxl":
                from modules.deft import load_ki_into_unet
                prompt = text_prompt_input
                
                pipeline.unet = load_ki_into_unet(torch.load(os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')), unet)
                ...
                from modules.deft import load_ki_into_unet
                prompt = text_prompt_input
                if os.path.exists(os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')):
                    print("Path exists.")
                else:
                ...
                if os.path.exists(os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')):
                    ...
                    print(f"Path {os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')} does not exist.")
                    continue
                ...
                prompt = text_prompt_input
                if os.path.exists(os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')):
                    print(f"Path exists {os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')}.")
                # else:
                #     print(f"Path {os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')} does not exist.")
                #     continue
                ...
                from modules.deft import load_ki_into_unet
                prompt = text_prompt_input
                ad_path = os.path.join(db_or_ti_output_dir, id, f'checkpoint-{steps}/project_replace.pt')
                breakpoint()
            ...
            if steps <= 500:
                for j, _output in enumerate(output):
                    save_image(cond_image, path=os.path.join(save_dir, f"progress_{steps}", method_dir, "src_image", id, f"{steps}_{i}_{j}.jpg"))
                    save_image(_output, path=os.path.join(save_dir, f"progress_{steps}", method_dir, "tgt_image", id, f"{steps}_{i}_{j}.jpg"))
                    with megfile.smart_open(os.path.join(save_dir, f"progress_{steps}", method_dir, "text", id, f"{steps}_{i}_{j}.txt"), "w") as f:
                        f.write(prompt_gt)

                pbar.update(1)
            else:
                for j, _output in enumerate(output):
                    save_image(cond_image, path=os.path.join(save_dir, method_dir, "src_image", id, f"{steps}_{i}_{j}.jpg"))
                    save_image(_output, path=os.path.join(save_dir, method_dir, "tgt_image", id, f"{steps}_{i}_{j}.jpg"))
                    with megfile.smart_open(os.path.join(save_dir, method_dir, "text", id, f"{steps}_{i}_{j}.txt"), "w") as f:
                        f.write(prompt_gt)
