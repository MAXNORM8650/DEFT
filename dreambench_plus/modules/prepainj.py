...
from .injection import KnowledgeInjectionAdapter, KnowledgeInjectionLayer, InjectionMethod
...
def make_knowledge_injection_adapter(model, config):
    ...
    def replace_modules(module, parent_name=""):
        for name, child in module.named_children():
            ...
            # Determine if this module should be targeted
            is_target = False
            if not config.target_modules:
                # Auto-target Linear layers
                is_target = isinstance(child, nn.Linear)
            else:
                # Check against target module list
            ...
            if is_target and isinstance(child, nn.Linear):
                try:
                    ...
                    # Remove irrelevant config keys
                    # keys_to_remove = ['target_modules']
    ...
    # Apply to model's LLM if it exists, otherwise the whole model
...
def add_knowledge_injection_methods(model):
    ...
    def load_knowledge_injection_adapter(model, adapter_path):
        ...
        # First, revert any existing KnowledgeInjectionLayers to base layers
        ...
        # Then apply new Knowledge Injection layers
        modified_count = 0
...
# 2. Create configuration for Knowledge Injection
from .injection import InjectionMethod
