...
class CLIPScore:
    ...
    def get_text_features(self, text: str | list[str], *, norm: bool = False) -> torch.Tensor:
        ...
        # text_features = self.model.get_text_features(inputs["input_ids"].to(self.device), inputs["attention_mask"].to(self.device))
        if norm:
...
def multigpu_eval_clipt_score(
    ...
    if clip_score is None:
        clip_score = CLIPScore(device=distributed_state.device)
    if subject is not None:
        text_files = megfile.smart_glob(os.path.join(text_dir, f"**/*{subject}*/**/*.txt"))
    else:
        text_files = megfile.smart_glob(os.path.join(text_dir, f"**/*.txt"))
    # breakpoint()
    ...
    for _ext in IMAGE_EXT:
        if subject is not None:
            image_files.extend(megfile.smart_glob(os.path.join(image_dir, f"**/*{subject}*/**/*.{_ext}")))
        else:
    ...
    with distributed_state.split_between_processes(params) as sub_params:
        ...
        for _param in sub_params:
            ...
            with megfile.smart_open(text_file, "r") as f:
                ...
                text = sentences[0]
                text = re.sub(r"The refernce image is <img><|image_d|></img>", "", text)
                text = re.sub(r"<img><|image_d|></img>", "", text)
                # sentences = [sentence for sentence in sentences if not re.search(r'<img><\|image_\d\|></img>', sentence)]
